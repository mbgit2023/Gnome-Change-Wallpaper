################################
## GNOME CHANGE WALLPAPER v.1 ##
##          by MB1            ##
################################

1. DESCRIPTION

GCW is a simple GUI that allows the user to change the background of the Gnome desktop at a specific interval.
It sets a new image by randomly choosing it from the selected folder. It also allows the user to enter an URL of the image that will be downloaded and set as background.

----------------------------------------------------------------------------------------------------------------
2. INSTALLATION

Extract the content of the archive and execute

    ./install.sh

Launch the application

    ./gnomechangewall.sh

----------------------------------------------------------------------------------------------------------------
3. AUTHOR

MB1

